  ...
  *PERINGATAN: Jadilah pemeriksa yang realistik. Karangan yang pendek dan menggunakan bahasa harian tidak layak mendapat gred A (15+).*`;
  ...
  try {
    ...
    if (typeof stimulus === 'string' && stimulus.startsWith('http')) {
      // Clean query strings (like Firebase tokens) out of the URL path before checking the extension
      const isPdf = stimulus.toLowerCase().split('?')[0].endsWith('.pdf');

      if (isPdf) {
        // Fetch remote URL PDF and render page 1 to an image buffer
