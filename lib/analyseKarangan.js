...
const LEVEL_SETTINGS = {
  'P3': { maxIsi: 8,  maxBahasa: 7,  total: 15, pics: 4 },
  'P4': { maxIsi: 8,  maxBahasa: 7,  total: 15, pics: 4 },
...
export async function analyseKarangan({
  ...
  ARAHAN OUTPUT JSON (WAJIB):
  {
    ...
    "kesalahanBahasa": [
      {
        "ayatSalah": "...",
        "pembetulan": "TULIS SEMULA SELURUH AYAT LENGKAP YANG TELAH DIBETULKAN (Untuk kegunaan Game)",
        "pembetulanPDF": "HANYA PERKATAAN YANG SALAH DITUKARKAN KEPADA YANG BETUL (Untuk kegunaan laporan PDF)",
        "kategori": "ejaan/tatabahasa/bahasa_pasar",
  ...
  *PERINGATAN: Jadilah pemeriksa yang realistik. Karangan yang pendek dan menggunakan bahasa harian tidak layak mendapat gred A (15+).*`;
  ...
  try {
    ...
    // Guardrails for Ali's case (Short/Touch-and-go)
    if ((level === 'P5' || level === 'P6') && wordCount < 110 && rawIsi > 12) {
      rawIsi = 12;
    }

    if (aiData.isIncomplete) rawIsi = Math.min(rawIsi, 4);
    else rawIsi = Math.max(9, rawIsi); // Complete story baseline
    ...
    return {
      ...
      markahIsi: Math.round(finalIsi),
      markahBahasa: Math.round(finalBahasa),
      markahKeseluruhan: Math.round(finalIsi + finalBahasa),
      ...
      ulasan: {
        ...
        keseluruhan: generateSmartUlasan(Math.round(finalIsi), Math.round(finalBahasa), peribahasaFound.length, combinedKesalahan.length, aiData.isIncomplete || false, level)
      },
    };
  } catch (err) {
