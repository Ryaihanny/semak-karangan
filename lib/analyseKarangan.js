...
// --- INTEGRATED CALCULATION LOGIC (REVISED) ---
    let rawIsi = aiData.markahIsiRaw || 10;
    let rawBahasa = aiData.markahBahasaRaw || 10;

    // 1. Peraturan Patah Perkataan (Hanya untuk P5/P6)
    if (level === 'P5' || level === 'P6') {
      if (wordCount < 80) {
        rawIsi = Math.min(rawIsi, 4); // 50-80 words: Max 4
      } 
      else if (wordCount < 100) {
        rawIsi = Math.min(rawIsi, 8); // 80-100 words: Max 8
      } 
      else if (wordCount < 150) {
        rawIsi = Math.min(rawIsi, 12); // 100-150 words: Max 12
      }
    }

    // 2. Scaling Logic (Menukar 20/20 kepada format 7/8 untuk P3/P4)
    let finalIsi, finalBahasa;

    if (level === 'P3' || level === 'P4') {
      // Tukar rawIsi (0-20) kepada (0-7)
      finalIsi = (rawIsi / 20) * config.maxIsi;
      // Tukar rawBahasa (0-20) kepada (0-8)
