  ...
  try {
    ...
    if (stimulus?.startsWith('http')) {
      ...
      stimulusPart = {
        inlineData: {
          data: base64Data,
          mimeType: isPdf ? "application/pdf" : "image/jpeg" // <--- Dynamic MimeType
        }
      };
    } else {
    ...
    - Nyatakan dalam 'analisisStimulus' kenapa ia dianggap terpesong.`;
...
const formattedParts = [
    ...
    for (const item of studentContent) {
      if (typeof item === 'string') {
        ...
        // Force application/pdf mimeType structure if it's a PDF document
