...
import sharp from 'sharp'; // Strictly used for image conversion
import pdfImgConvert from 'pdf-img-convert'; // Strictly used for PDF conversion
...
export async function analyseKarangan({
  ...
  2. ANALISIS BAHASA (0-20 markah):
     - SYARAT MINIMA (10-12): Bahasa boleh difahaman walaupun ada kesalahan ejaan/imbuhan.
  ...
  *PERINGATAN: Jadilah pemeriksa yang realistik. Karangan yang pendek dan menggunakan bahasa harian tidak layak mendapat gred A (15+).*`;
  ...
  try {
    ...
    // ✨ SAFELY ISOLATED PORTION: Carefully handles alternative URL extensions & files
    if (typeof stimulus === 'string' && stimulus.startsWith('http')) {
      const cleanUrlPath = stimulus.split('?')[0].toLowerCase();
      const isPdf = cleanUrlPath.endsWith('.pdf');

      if (isPdf) {
        // Fetch remote URL PDF and render page 1 to an image buffer safely
        const response = await fetch(stimulus);
        const arrayBuffer = await response.arrayBuffer();
        const convertedPages = await pdfImgConvert.convert(Buffer.from(arrayBuffer), { width: 1200 });
        const optimizedBuffer = await sharp(Buffer.from(convertedPages[0]))
          .jpeg({ quality: 70 })
          .toBuffer();
        ...
        const base64Data = await fetchImageAsBase64(stimulus);
        stimulusPart = {
          inlineData: {
            data: base64Data,
            mimeType: "image/jpeg"
      ...
      stimulusPart = { text: `Stimulus Soalan: ${stimulus || "Tiada stimulus."}` };
    }

    const promptWithVigilance = prompt + `
    
    PERINGATAN TAMBAHAN (ISI TERPESONG):
    - Bandingkan karangan murid dengan gambar/PDF stimulus secara visual.
    - Jika karangan murid menceritakan situasi yang SEPENUHNYA BERBEZA daripada gambar (Out of Point), anda WAJIB memberikan Markah Isi bawah 5/20.
    - Nyatakan dalam 'analisisStimulus' why it is considered out of point.`;

    const formattedParts = [
      { text: promptWithVigilance },
      stimulusPart
    ];

    // Safely structure incoming text strings, images, or PDFs for Gemini
    for (const item of studentContent) {
      if (typeof item === 'string') {
        formattedParts.push({ text: item });
      } else if (item && item.inlineData) {
        const currentMime = item.inlineData.mimeType;
        if (currentMime === 'application/pdf') {
          // Flatten student assignment PDF pages into raw image data frames
          const rawPdfBuffer = Buffer.from(item.inlineData.data, 'base64');
          const convertedPages = await pdfImgConvert.convert(rawPdfBuffer, { width: 1200 });
          ...
          formattedParts.push(item); // Keep images as image/jpeg
        }
      } else if (item) {
        formattedParts.push(item);
      }
    }
