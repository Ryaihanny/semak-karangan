...
export default function AdminProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert('Profil Admin berjaya dikemaskini!');
    } catch (err) {
      console.error(err);
    ...
    <div className="dashboard-wrapper">
      ...
      <aside className="main-sidebar">
        <div className="sidebar-logo">
          ...
          <div className="logo-text"><h3>SI-PINTAR</h3><span>VERSI ADMIN</span></div>
        </div>

        <nav className="sidebar-nav">
          <div className="nav-header">PENTADBIRAN</div>
          <div className="nav-link" onClick={() => router.push('/admin/dashboard')}>📊 Admin Panel</div>
          <div className="nav-link" onClick={() => router.push('/admin/users')}>👥 Urus Pengguna</div>
          <div className="nav-link" onClick={() => router.push('/admin/urus-kelas')}>🏫 Urus Kelas</div>
          
          <div className="nav-divider"></div>

          <div className="nav-header">PENGURUSAN DIRI</div>
          <div className="nav-link active">👤 Profil Admin</div>
          <div className="nav-link" onClick={() => router.push('/dashboard')}>⬅️ Ke Panel Guru</div>
          
          <div className="nav-divider"></div>

          <div className="nav-action-zone">
            <div className="nav-link highlight" onClick={() => router.push('/semak')}>✍️ Uji Semakan</div>
          </div>
        </nav>
      ...
      <main className="main-viewport">
        <header className="viewport-header">
          <h1>Profil Pentadbir (Admin)</h1>
          <div className="admin-badge">Status: <b>ADMINISTRATOR</b></div>
        </header>
        ...
        <div className="fade-in">
          <div className="pro-card" style={{ maxWidth: '600px' }}>
            <div className="profile-edit-form">
              ...
              <div className="t-group" style={{ marginBottom: '20px' }}>
                <label>Sekolah / Institusi Utama</label>
                <input 
                  type="text" 
                  value={sekolah} 
                  onChange={(e) => setSekolah(e.target.value)} 
                  placeholder="Nama pusat"
                />
              </div>

              <div className="t-group" style={{ marginBottom: '30px' }}>
                <label>E-mel Admin (Kekal)</label>
                <input 
                  type="text" 
              ...
              <button 
                ...
                disabled={saving}
              >
                {saving ? 'Menyimpan...' : 'Kemaskini Profil Admin'}
              </button>
            </div>
          </div>
        </div>
      ...
      <style jsx>{`
        ...
        .admin-badge { background: #003D40; color: white; padding: 8px 16px; border-radius: 50px; font-size: 0.75rem; letter-spacing: 1px; }
